...
[Angelopoulos et al., 2023b](arxiv.org/abs/2311.01453), and post-prediction 
adaptive inference (PSPA) by 
[Miao et al., 2023](https://arxiv.org/abs/2311.14220). To enable researchers 
and practitioners interested in these state-of-the-art methods, we have 
developed `ipd`, a open-source `R` package that implements these methods under 
the umbrella of IPD.
...
```{r plot, echo=FALSE, comment = NA, fig.height=4, fig.width=7}
library(tidyverse)
...
p1 <- dat_labeled |>
    ...
    mutate(
        ...
        name = factor(name, c("f", "Y"), c("Predicted ", "Observed ")) |> 
...
-   The predicted outcomes are not perfect substitutes for the 
true outcomes (plot B)
...
```{r plot2, echo=FALSE, fig.width=7}
...
We compare two non-IPD approaches to analyzing the data to methods included in 
the `ipd` package.

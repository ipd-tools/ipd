...
dat_logistic_labeled_summ |>
  ...
  geom_text(
    ...
    aes(label = paste0(n, " (", sprintf("%2.1f", pct), "%)")),
    vjust = 1
  ) +
...
ipd::ipd(formula,
  method = "postpi_boot", model = "logistic",
...
ipd::ipd(formula,
  method = "ppi_plusplus", model = "logistic",
  data = dat_logistic, label = "set_label"
) |>
  summary()
```

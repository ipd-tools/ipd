...
```{r plot, echo = F, error = F, warning = F, message = F, comment = NA, fig.height=4}
...
fit0 <- lm(f ~ X1, data = dat[dat$set == "unlabeled",]) |> summary()
fit1 <- lm(Y ~ X1, data = dat[dat$set == "labeled",])   |> summary()

formula <- Y - f ~ X1

nboot <- 200

fit2 <- IPD::ipd(formula, method = "postpi_boot", model = "ols", 
                 data = dat, label = "set", nboot = nboot) |> summary()

fit3 <- IPD::ipd(formula, method = "postpi_analytic", model = "ols", 
                 data = dat, label = "set") |> summary()

fit4 <- IPD::ipd(formula, method = "ppi", model = "ols", 
                 data = dat, label = "set") |> summary()

fit5 <- IPD::ipd(formula, method = "ppi_plusplus", model = "ols", 
                 data = dat, label = "set") |> summary()

fit6 <- IPD::ipd(formula, method = "pspa", model = "ols", 
                 data = dat, label = "set") |> summary()

sumtab <- rbind(
  
  fit0$coefficients[2, 1:2],
  fit1$coefficients[2, 1:2],
  fit2$coefficients[2, 1:2],
  fit3$coefficients[2, 1:2],
  fit4$coefficients[2, 1:2],
  fit5$coefficients[2, 1:2])

rownames(sumtab) <- c("Naive", "Classic", "PostPI (Bootstrap)", 

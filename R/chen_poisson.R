...
#' \describe{
#'    \item{est}{(vector): vector of Chen & Chen Poisson regression coefficient
#'    estimates.}
...
#'
#' @export
chen_poisson <- function(X_l, Y_l, f_l, X_u, f_u, intercept = TRUE) {
  if (!intercept) stop("ipd currently supplies design matrices with intercept; set intercept=TRUE.", call. = FALSE)

  X_a <- rbind(X_l, X_u)

  beta_lab  <- .fit_glm_from_matrix(Y_l, X_l, family = "poisson")
  gamma_lab <- .fit_glm_from_matrix(f_l, X_l, family = "poisson")
  gamma_all <- .fit_glm_from_matrix(c(f_l, f_u), X_a, family = "poisson")
